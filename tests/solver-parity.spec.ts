...
test('fixed-seed Chapter 4 route matches the live engine at every action boundary', async ({ page }) => {
  test.setTimeout(180_000);
  const scenario = JSON.parse(
    await readFile('solver/scenarios/chapter-4.json', 'utf8'),
  ) as SolverScenario;
  const saved = JSON.parse(
    await readFile('solver/solutions/chapter-4.json', 'utf8'),
  ) as SolverResult;
  ...
  for (let index = 0; index < route.plan!.length; index++) {
    ...
    try {
      ...
      throw new Error(`live replay failed after action ${index + 1}: ${JSON.stringify(action)}`, { cause: error });
    }
    ...
    expect(differences, `parity drift after action ${index + 1}: ${JSON.stringify(action)}`).toEqual([]);
  }
});
